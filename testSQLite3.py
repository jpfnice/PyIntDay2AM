...
try:
    ...
    # cursor.execute("create table temperatures (city varchar(20), currentdate date, temp float)")
    ...
    cursor.execute("select * from temperatures where temp < 2.5")
     
    while True:
        row = cursor.fetchone()
        if row == None:
            break
        print(f"{row[0]:12s}, {row[1]}, {row[2]}")
